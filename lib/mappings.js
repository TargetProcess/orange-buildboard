...
mappings = _.indexBy([
    {
        ...
        map(item, config){
            return {
                [config.sourceField]: {
                    $regex: config.regex.replace(config.placeholder, get(item, config.destField)),
...
function findItems(mapping, account, sourceItems) {
    var $or = _.compact(sourceItems.map(item=>mappings[mapping.id].map(item, mapping.config)));
    if ($or.length > 0) {
        return collections[mapping.destination].find({
            account,
            $or: $or
        });
    }
    else {
        return collections[mapping.destination].find({'nothing': 1});
    }
}
