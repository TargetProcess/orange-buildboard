...
function post(collection) {
    return function () {
        ...
        applyMapping({account: item.account, collection, item, modification: result.insertedId ? 'added' : 'changed'});
    };
}
...
function deleteSingle(collection) {
    return function () {
        ...
        if (item) {
            applyMapping({account: item.account, collection, item, modification: 'removed'});
